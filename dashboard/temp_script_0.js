...
Promise.all([
    fetch('/transcripts.json?v=22').then(r => {
        if (!r.ok) throw new Error("HTTP error " + r.status);
        return r.text();
    }),
    fetch('/quran_db.json?v=1').then(r => {
        if (!r.ok) return {};
        return r.json();
    }).catch(() => ({}))
])
.then(([transcriptsText, quranDb]) => {
    ...
    // Restore active tab from CloudStorage
    try {
        if (tg && tg.CloudStorage) {
            tg.CloudStorage.getItem('active_tab', (err, value) => {
                if (!err && value) {
                    switchTab(value);
                }
            });
    ...
    .catch(e => {
        ...
        const errorHtml = `<div class="empty-state" style="border-color:#ef4444;color:#ef4444">
            ...
            <p>فشل تحميل البيانات. الرجاء التحقق من الاتصال بالإنترنت. (Erreur: ${e.message})</p>
            <button class="btn-sm btn-primary" style="margin-top:10px;background:#ef4444" onclick="location.reload()">إعادة المحاولة</button>
        </div>`;
        const dyk = document.getElementById('did-you-know');
        if (dyk) dyk.innerHTML = errorHtml;
        const sg = document.getElementById('subject-grid');
        if (sg) sg.innerHTML = errorHtml;
        const rc = document.getElementById('recent-cards');
        if (rc) rc.innerHTML = errorHtml;
    });
...
function switchTab(name, btn) {
    ...
    if (btn) {
        ...
        const targetBtn = document.querySelector(`.nav-btn[onclick="switchTab('${name}',this)"]`);
    ...
    try {
        if(tg && tg.CloudStorage) tg.CloudStorage.setItem('active_tab', name);
    } catch(e) {}
}
