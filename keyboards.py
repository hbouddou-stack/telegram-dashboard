...
def get_main_inline_keyboard(is_admin: bool = False, remaining_count: int = None, unread_count: int = 0) -> InlineKeyboardMarkup:
    ...
    if base_url.startswith("https"):
        rows.append([
            InlineKeyboardButton(text="ðŸ–¥ï¸ Ø§Ù„Ù…Ù†ØµØ© Ø§Ù„ØªØ¹Ù„ÙŠÙ…ÙŠØ© ÙˆØ§Ù„Ù„Ù‘ÙŠØ³ÙˆØ² ðŸ“±", web_app=WebAppInfo(url=f"{base_url}/index.html?v=12"))
        ])
...
def get_admin_panel_keyboard(pending_reports: int = 0, pending_proposals: int = 0, show_settings: bool = False, role: str = None) -> InlineKeyboardMarkup:
    ...
    if base_url.startswith("https"):
        web_app_buttons.append(InlineKeyboardButton(text="ðŸ–¥ï¸ Ø§Ù„Ù…Ù†ØµØ© Ø§Ù„ØªØ¹Ù„ÙŠÙ…ÙŠØ© ÙˆØ§Ù„Ù„Ù‘ÙŠØ³ÙˆØ² ðŸ“±", web_app=WebAppInfo(url=f"{base_url}/index.html?v=12")))
