...
window.addEventListener('unhandledrejection', function(e) {
    ...
    document.body.appendChild(errorDiv);
        ...
        function applyDashboardModeText() {
            ...
            if (profile && !document.getElementById('dashboard-mode-switcher')) {
                profile.insertAdjacentHTML('beforeend', `
                    <div id="dashboard-mode-switcher" style="display:flex; gap:6px; width:100%; margin-top:10px;">
                        <a href="/admin-bot" class="btn btn-secondary" style="flex:1; justify-content:center; padding:7px 6px; font-size:0.72rem; text-decoration:none; ${state.dashboardScope === 'bot' ? 'border-color: var(--gold); color: var(--gold-light);' : ''}">Bot</a>
                        <a href="/admin-support" class="btn btn-secondary" style="flex:1; justify-content:center; padding:7px 6px; font-size:0.72rem; text-decoration:none; ${state.dashboardScope === 'support' ? 'border-color: var(--gold); color: var(--gold-light);' : ''}">Support</a>
                    </div>
                `);
            }
        }
        ...
        function updateStats() {
            ...
            const pendingTotal = state.pendingReports.filter(r => r.status === 'pending').length + state.pendingProposals.filter(p => p.status === 'pending').length + (state.tickets || []).filter(t => t.status === 'pending').length;

            document.getElementById('stat-pending-items').textContent = pendingTotal;

            document.getElementById('stat-active-users').textContent = "نشط";
                    ...
                    <div class="settings-panel" style="display: flex; flex-direction: row; gap: 20px; padding: 20px; min-height: 500px; border-top:none; margin-top:-20px; flex-wrap: wrap;">
                        ...
                        <div style="width: 300px; flex-shrink: 0; border-left: 1px solid var(--border); padding-left: 20px; display:flex; flex-direction:column; gap:10px;">
                            ...
                            <div class="factory-form-group">
                                ...
                                <label>🏷️ الموضوع / التهميش الرئيسي :</label>
                                            ...
                                            <div style="display:flex; flex-direction:column; gap:10px;">
                                                ...
                                                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:15px; margin-top:3px;">
                                                    ...
                                                    <div style="display:flex; flex-direction:column; gap:4px;">
                                                        ...
                                                        <span style="font-size:0.8rem; font-weight:600; color:var(--text-secondary);">الموضوع الفرعي :</span>

                                                        <input type="text" class="factory-input" value="${q.theme || q.sub_theme || ''}" oninput="state.generatedQuestions[${qIdx}].theme = this.value;">
        ...
        function renderQuestionsGrid() {
            ...
            courses.forEach(cNum => {
                ...
                card.innerHTML = `
                    ...
                    <div class="qb-course-card-total" style="font-size:1.2rem; margin:2px 0;">${cData.total} <span style="font-size: 0.75rem; font-weight: 500; color: var(--text-secondary);">أسئلة</span></div>
            ...
            if (chapters.length === 0) {
                ...
                chapters.forEach(ch => {
                    ...
                    html += `
                        ...
                        <div class="qb-thematic-item" style="flex-direction: column; align-items: stretch; gap: 8px; padding: 12px; border-radius: var(--radius-sm); border: 1px solid var(--border); background: var(--bg); ${ch.count === 0 ? 'border-right: 3px solid #ef4444;' : 'border-right: 3px solid ' + (SUBJECT_COLORS[selectedSubject] || 'var(--border)')}">
        ...
        async function toggleQuestionActive(questionId, checkbox) {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    showNotification(isActive ? 'تم تفعيل السؤال بنجاح' : 'تم إخفاء السؤال بنجاح', 'success');
