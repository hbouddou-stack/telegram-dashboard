...
async def handle_interactive(request):
    return web.FileResponse('dashboard/interactive.html')

async def handle_admin(request):
    return web.FileResponse('dashboard/admin.html')

async def handle_support(request):
    return web.FileResponse('dashboard/support.html')

async def handle_search(request):
    return web.FileResponse('dashboard/search.html')

async def handle_transcripts(request):
    transcripts_path = 'dashboard/transcripts.json'
    if not os.path.exists(transcripts_path):
        transcripts_path = 'dashboard/transcripts.json.bak'
    return web.FileResponse(transcripts_path)

async def handle_quran(request):
    return web.FileResponse('dashboard/quran_db.json')
...
async def get_admin_info(request):
    try:
        ...
        allowed_subjects = _json.loads(allowed_subjects_raw) if allowed_subjects_raw else None
...
async def generate_questions_ia(request):
    try:
        ...
        # Reload GEMINI_API_KEY fresh from config (ensures .env was loaded)
        import importlib
        import config as cfg_module
        importlib.reload(cfg_module)
        GEMINI_API_KEY = cfg_module.GEMINI_API_KEY
        ...
        prompt = f"""أنت خبير في الشريعة الإسلامية ومصمم اختبارات تعليمية.
    ...
    try:
        ...
        for q in questions:
            q_data = {
                ...
                "source": "ai_generated",
                "hijra_year": None,
                "theme": q.get("theme") or q.get("sub_theme") or ""
            }
...
async def get_admin_question(request):
    try:
        ...
        async with aiosqlite.connect(DATABASE_PATH) as db_conn:
            db_conn.row_factory = aiosqlite.Row
            async with db_conn.execute("""
                SELECT id, subject, course_number, course_name, question,
                       choice_a, choice_b, choice_c, choice_d, correct_answer,
                       explanation, source, created_at, hijra_year, theme, is_active
                FROM questions
                WHERE id = ?
            """, (int(question_id),)) as cur:
                row = await cur.fetchone()
        ...
        if q_dict.get("source") == "student_proposal":
            async with aiosqlite.connect(DATABASE_PATH) as db_conn:
                ...
                async with db_conn.execute("""
                    ...
                    LIMIT 1
                """, (q_dict["question"], q_dict["subject"], q_dict["course_number"], q_dict["question"])) as cur:
...
async def update_admin_question(request):
    try:
        ...
        async with aiosqlite.connect(DATABASE_PATH) as db_conn:
            await db_conn.execute("""
                UPDATE questions
                SET question = ?, choice_a = ?, choice_b = ?, choice_c = ?,
                    choice_d = ?, correct_answer = ?, explanation = ?, is_active = ?
                WHERE id = ?
            """, (question, choice_a, choice_b, choice_c, choice_d, correct_answer, explanation, is_active, int(question_id)))
            await db_conn.commit()
...
async def update_admin_proposal(request):
    try:
        ...
        async with aiosqlite.connect(DATABASE_PATH) as db_conn:
            await db_conn.execute("""
                UPDATE questions_proposees
                SET question = ?, choice_a = ?, choice_b = ?, choice_c = ?,
                    choice_d = ?, correct_answer = ?, explanation = ?
                WHERE id = ?
            """, (question, choice_a, choice_b, choice_c, choice_d, correct_answer, explanation, int(proposal_id)))
            await db_conn.commit()
...
async def get_admin_proposals(request):
    try:
        ...
        async with aiosqlite.connect(DATABASE_PATH) as db_conn:
            db_conn.row_factory = aiosqlite.Row
            async with db_conn.execute("""
                SELECT qp.id, qp.user_id, qp.username, qp.first_name, qp.subject, qp.topic, qp.lesson, qp.question, 
                       qp.choice_a, qp.choice_b, qp.choice_c, qp.choice_d, qp.correct_answer, qp.explanation, qp.status, qp.admin_feedback, qp.created_at,
                       u.academic_year, qp.source, qp.contact_info, qp.claimed_by, qp.tags, qp.media_file_id, qp.media_type
                FROM questions_proposees qp
                LEFT JOIN users u ON qp.user_id = u.telegram_id
    ...
    try:
        ...
        async with aiosqlite.connect(DATABASE_PATH) as db_conn:
            ...
            if action == 'approved':
                ...
                await db_conn.execute("""
                    INSERT INTO questions (id, subject, course_number, course_name, question, choice_a, choice_b, choice_c, choice_d, correct_answer, explanation, source)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'student_proposal')
                """, (next_id, prop_details["subject"], prop_details["course_number"], prop_details["lesson"], prop_details["question"],
                      prop_details["choice_a"], prop_details["choice_b"], prop_details["choice_c"], prop_details["choice_d"], prop_details["correct_answer"], prop_details["explanation"]))
...
async def get_admin_tickets(request):
    try:
        ...
        async with aiosqlite.connect(DATABASE_PATH) as db_conn:
            ...
            query = """
                SELECT qr.id, qr.user_id, qr.username, qr.first_name, qr.report_type, qr.question_id,
                       qr.target, qr.notes, qr.urgency, qr.status, qr.admin_reply, qr.reviewed_at, qr.created_at,
                       qr.claimed_by, qr.student_read, qr.tags, qr.media_file_id, qr.media_type,
                       q.subject AS question_subject, q.course_number AS question_course_number,
                       u.academic_year, qr.source, qr.contact_info
                FROM question_reports qr
                LEFT JOIN questions q ON qr.question_id = q.id
                LEFT JOIN users u ON qr.user_id = u.telegram_id
                WHERE 1=1
            """
    ...
    try:
        ...
        async with aiosqlite.connect(DATABASE_PATH) as db_conn:
            cursor = await db_conn.execute("""
                INSERT INTO question_reports (user_id, username, first_name, report_type, notes, urgency, source, contact_info)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (telegram_id, username, first_name, report_type, notes, "Moyen", source, contact_info))
            await db_conn.commit()
...
async def get_student_tickets(request):
    try:
        ...
        async with aiosqlite.connect(DATABASE_PATH) as db_conn:
            db_conn.row_factory = aiosqlite.Row
            async with db_conn.execute("""
                SELECT id, report_type, notes, urgency, status, admin_reply, created_at, media_file_id, media_type
                FROM question_reports
                WHERE user_id = ?
                ORDER BY created_at DESC
