...
async def api_admin_gateway_home_stats(request: web.Request):
    ...
    try:
        async with aiosqlite.connect(DATABASE_PATH) as db:
            ...
            async with db.execute("SELECT COUNT(*) as cnt FROM users u LEFT JOIN academy_students s ON s.telegram_id = u.telegram_id GROUP BY u.telegram_id WHERE s.telegram_id IS NULL AND (u.excluded = 0 OR u.excluded IS NULL) AND (u.excluded = 0 OR u.excluded IS NULL)") as cur:
            ...
            # Ghosts from today
            async with db.execute("SELECT COUNT(*) as cnt FROM users u LEFT JOIN academy_students s ON s.telegram_id = u.telegram_id GROUP BY u.telegram_id WHERE s.telegram_id IS NULL AND (u.excluded = 0 OR u.excluded IS NULL) AND (u.excluded = 0 OR u.excluded IS NULL) AND u.created_at >= date('now')") as cur:
...
async def api_admin_gateway_ghost_visitors(request: web.Request):
    ...
    try:
        ...
        async with aiosqlite.connect(DATABASE_PATH) as db:
            ...
            if status == 'sos':
                base_select = """
                    ...
                    LEFT JOIN academy_students s ON s.telegram_id = u.telegram_id GROUP BY u.telegram_id
                    WHERE g.status = 'open' GROUP BY u.telegram_id
                """
                if search:
                    st = search.replace("'", "''")
                ...
                if search:
                    base_select += f" AND (LOWER(s.first_name) LIKE '%{search}%' OR LOWER(s.last_name) LIKE '%{search}%' OR LOWER(s.email) LIKE '%{search}%')"
                base_select += " ORDER BY s.created_at DESC LIMIT 500"
            else:
                base_select = """
                    SELECT 
                        u.telegram_id, u.first_name, u.last_name, u.username, u.created_at,
                ...
                filter_clause = ""
                if gender and gender != 'all':
                    # Need to check users.first_name/last_name heuristics or student.gender if linked
                    pass # Keep simple for ghosts, skip exact gender filter for pure users to avoid complexity
                if search:
                    st = search.replace("'", "''")
        ...
        return web.json_response({'success': True, 'visitors': rows, 'counts': counts, 'status': status})
    except Exception as e:
