...
async def on_startup(bot: Bot):
    ...
    try:
        ...
        faq_data = [
            ...
            ("التسجيل والأسعار", "عام", "ماذا يحدث بعد التسجيل مباشرة؟", "بمجرد إتمام الدفع وتأكيد التسجيل، سيتم منحك حق الوصول الفوري إلى فضاء الطالب الخاص بك للبدء في مشاهدة الدروس."),
            
            # Curriculum & Study Method
            ("المنهج والدراسة", "الوقت", "لا أملك إلا 3 ساعات أسبوعياً، هل هذا يكفي؟", "نعم، بتخصيص حوالي نصف ساعة يومياً أو 3 ساعات أسبوعياً يمكنك متابعة الدروس والمراجعة دون أن تتأخر عن زملائك."),
            ("المنهج والدراسة", "المستويات", "أبدأ من الصفر، هل سأفهم الدروس؟", "بالتأكيد، المنهج مصمم بشكل متدرج يبدأ من المستوى الأساسي (التأسيس) ويبني معك المعرفة خطوة بخطوة دون افتراض معرفة مسبقة."),
            ("المنهج والدراسة", "المستويات", "كم عدد المستويات أو الفصول الدراسية؟", "البرنامج يتكون من 4 سنوات مقسمة إلى أربعة مستويات متدرجة: التأسيس، البناء، التمكّن، والإتقان."),
            ("المنهج والدراسة", "المستويات", "ماذا أدرس في مستوى التأسيس؟", "تدرس مبادئ السيرة، العقيدة، الفقه، النحو، التجويد، مع التزكية والأخلاق وآداب طالب العلم وحفظ القرآن لتأسيسك علمياً ومنهجياً."),
            ("المنهج والدراسة", "المستويات", "ماذا أدرس في مستوى البناء؟", "تتوسع في الفقه والعقيدة والنحو والتجويد، وتدرس مصطلح الحديث وأصول الفقه لبناء ملكة علمية متماسكة."),
            ("المنهج والدراسة", "المستويات", "ماذا أدرس في مستوى التمكن؟", "تنتقل إلى فقه الحديث، قراءة نافع، الصرف، البلاغة، والقواعد الفقهية للارتقاء نحو الفهم العميق والاستنباط."),
            ("المنهج والدراسة", "المستويات", "ماذا أدرس في مستوى الإتقان؟", "تستكمل البناء بفقه الحديث، علوم القرآن، تاريخ التشريع، مقاصد الشريعة، ودراسة الإلحاد المعاصر ليخرج الطالب مؤصلاً في العلم."),
            ("المنهج والدراسة", "الشهادات", "هل هناك شهادة تخرج واختبارات؟", "نعم، هناك اختبارات دورية في نهاية كل مستوى لتقييم الاستيعاب، ويحصل الطالب على شهادة تخرج بعد إتمام البرنامج كاملًا."),
            ("المنهج والدراسة", "المواد", "ما هي المواد التي تُدرس في الأكاديمية؟", "نُدرس 20 مادة متكاملة منها: السيرة النبوية، التجويد، التوحيد، النحو، الفقه المالكي، مصطلح الحديث، أصول الفقه، البلاغة، علوم القرآن، وغيرها."),
            ("المنهج والدراسة", "المذهب", "على أي مذهب تُدرس مادة الفقه؟", "تُدرس مادة الفقه وفق المذهب المالكي تأصيلاً وتدريجاً من خلال أمّهات كتب المذهب ومتونه المعتمدة."),
            ("المنهج والدراسة", "المراجعة", "هل توجد ملخصات للدروس؟", "نعم، نوفر تفريغات وملخصات ومشجرات علمية بعد كل وحدة لتسهيل المراجعة وترسيخ الفهم."),
            
            # Teachers
            ("هيئة التدريس", "عام", "من هم مشايخ الأكاديمية؟", "نخبة من المشايخ المجازين منهم: د. البشير عصام، ذ. ياسين العمري، ذ. يونس بوعوام، ذ. نور الدين درواش، ذ. محمد شوام، وغيرهم."),
            ("هيئة التدريس", "المواد", "من يدرس مادة السيرة النبوية؟", "يقوم بتدريس مادة السيرة النبوية الأستاذ ياسين العمري."),
            ("هيئة التدريس", "المواد", "من يدرس مادة النحو وأصول الفقه؟", "الدكتور البشير عصام هو من يدرس النحو وأصول الفقه بالإضافة لمقاصد الشريعة وتاريخ التشريع."),
            ("هيئة التدريس", "المواد", "من يدرس مادة الفقه؟", "يقوم الأستاذ يونس بوعوام بتدريس الفقه للمستويات الثلاثة الأولى، والأستاذ نور الدين درواش للمستوى الرابع."),
            ("هيئة التدريس", "الأسئلة", "هل يمكنني طرح أسئلة على المشايخ؟", "نعم، يمكنك طرح إشكالاتك العلمية وتُجاب مباشرة من المشايخ حتى لا يبقى موضع لبس دون بيان."),

            # Classes and Platform
            ("الدروس والمنصة", "عام", "هل الدروس مباشرة أم مسجلة؟", "النظام يجمع بين الاثنين: دروس مسجلة بجودة عالية تتابعها متى شئت، ولقاءات مباشرة مع المشايخ لطرح الأسئلة والتوجيه."),
            ("الدروس والمنصة", "عام", "ماذا لو فاتني درس مباشر؟", "لا تقلق، جميع الدروس واللقاءات محفوظة ومسجلة ويمكنك العودة إليها في أي وقت لمراجعتها."),
            ("الدروس والمنصة", "عام", "هل هناك أنشطة إضافية غير الدروس الأساسية؟", "نعم، نقدم دورات علمية متخصصة ومحاضرات إثرائية دورية في مجالات تربوية ومعرفية متنوعة لتوسيع مدارك الطالب."),
            ...
            ("تواصل ودعم", "تقني", "ماذا أفعل إن واجهت مشكلة تقنية في المنصة؟", "لدينا فريق دعم فني متواجد طوال فترة الاشتراك لمساعدتك، يمكنك مراسلتنا عبر الواتساب وسنقوم بحل المشكلة فوراً."),
            
            # Irshad Center System
            ("مشاريع أخرى", "عام", "هل لديكم برامج للأطفال والناشئة؟", "نعم، لدينا (أكاديمية فتية) التي تعنى بالتأسيس الشرعي واللغوي للفتية والصغار."),
            ("مشاريع أخرى", "عام", "كيف أتعلم حفظ القرآن وتجويده فقط؟", "يمكنك الالتحاق بمنصة (المراقي) التابعة لمركز إرشاد، وهي مخصصة لتحفيظ القرآن الكريم وتعليم التجويد عن بُعد."),
            ("مشاريع أخرى", "عام", "هل هناك مشروع متخصص لتعليم القراءات؟", "نعم، (أكاديمية بناء المقرئ) وهو مشروع علمي رائد لتعليم القراءات القرآنية ومنح الإجازات بأسانيد متصلة.")
        ]
        async with aiosqlite.connect(DATABASE_PATH) as conn:
