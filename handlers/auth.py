...
async def cmd_png(message: Message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📊 لوحة تحكم المشرفين (Gateway Admin)", web_app=WebAppInfo(url=f"{base_url}/admin_gateway.html?v=live_admin_v8"))]
    ])
...
async def cmd_federer(message: Message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📊 لوحة تحكم المشرفين وإدارة الطلاب (Gateway Admin)", web_app=WebAppInfo(url=f"{base_url}/admin_gateway.html?v=live_admin_v8"))],
