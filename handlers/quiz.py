...
async def render_mode_selection(callback: CallbackQuery, state: FSMContext, subject: str):
    ...
    if subject == "sira":
        text = (
            f"<blockquote>🎯 <b>مادة {subject_ar}</b></blockquote>\n\n"
...
async def render_lessons_grid(callback: CallbackQuery, state: FSMContext, subject: str, selected_lessons: list[int]):
    ...
    await callback.message.edit_text(
        f"<blockquote>📖 <b>مادة {subject_ar} - اختيار الدروس</b></blockquote>\n\n"
        f"{legend}"
        f"{prompt_text}",
...
async def render_themes_grid(callback: CallbackQuery, state: FSMContext, subject: str, selected_themes: list[int]):
    await state.set_state(QuizStates.selecting_themes)
    subject_ar = SUBJECT_MAP.get(subject, subject)
    
    available_themes = await db.get_available_themes(subject)
    await state.update_data(available_themes=available_themes)
        
    await callback.message.edit_text(
        f"<blockquote>🎯 <b>مادة {subject_ar} - اختيار المحاور</b></blockquote>\n\n"
...
async def render_exam_settings(callback: CallbackQuery, state: FSMContext, subject: str, mode: str, selection_text: str, settings: dict):
    ...
    text = (
        f"<blockquote>⚙️ <b>إعدادات التمرين - {subject_ar} ({mode_ar})</b></blockquote>\n\n"
...
async def show_question(callback: CallbackQuery, state: FSMContext):
    ...
    # Subject and Theme
    theme_str = f" | 📚 المحور: {q.get('theme')}" if q.get("theme") else ""
    
    footer_text = (
        f"📝 <b>السؤال {current_index + 1}/{len(questions)}</b> | <b>الدرس {q.get('course_number')}</b>{mastered_str}\n"
        f"({subject_ar}){theme_str}\n"
        f"📊 {progress_bar} {int(progress_ratio * 100)}%"
    )
    text += f"<blockquote>{footer_text}</blockquote>"
...
async def handle_subject_selected(callback: CallbackQuery, state: FSMContext):
    ...
    # Automatically update the preferred subject to match the selected one
    await db.update_user_preferred_subject(user_id, subject)
...
async def handle_select_theme(callback: CallbackQuery, state: FSMContext):
    ...
    if theme_val.isdigit():
        theme_id = int(theme_val)
        available_themes = data.get("available_themes", [])
        theme_dict = next((t for t in available_themes if t['id'] == theme_id), None)
        if not theme_dict:
            await callback.message.answer("⚠️ حدث خطأ في اختيار المحور.")
            return
        theme_title = theme_dict['title']
    else:
        theme_id = theme_val
        theme_title = str(theme_val)
        
    # Skip sub-themes for all subjects (Solution A)
    await state.update_data(selected_theme=theme_id, selected_sub_themes=[])
    
    settings = data.get("settings", get_default_settings(subject))
    await state.update_data(settings=settings, selected_themes=[theme_id])
    await render_exam_settings(callback, state, subject, "themes", theme_title, settings)
...
async def render_sub_themes_grid(callback: CallbackQuery, state: FSMContext, subject: str, theme: str, selected_sub_themes: list[str]):
    ...
    await state.update_data(available_sub_themes=available_sub_themes)
...
async def initialize_and_start_quiz(callback: CallbackQuery, state: FSMContext, data: dict):
    ...
    if subject == "sira" and mode == "years":
        ...
        questions = await db.get_questions_for_theme_nodes(selected_themes)
    else:
    ...
    # Solution A: We aggregate all Level 3 questions automatically so no filtering is needed.
    pass
            
    # Force origin: lessons -> official only, themes -> practice/training only
    # Exception: Tajweed questions are generated by AI, so they use 'generated_by_gemini' source
    origin_val = "practice" if mode == "themes" else "official"
    if origin_val == "official" and questions:
        if subject == "tajweed":
            # Tajweed questions are AI-generated, accept both official and generated sources
            questions = [q for q in questions if q.get("source") in ["official", None, "", "generated_by_gemini", "ai"]]
        else:
...
async def handle_prof_quote(callback: CallbackQuery, state: FSMContext):
    ...
    user_id = callback.from_user.id
    is_fav = await db.is_favorite(user_id, q_id)
    fav_text = "⭐ أزل من المفضلة" if is_fav else "⭐ أضف للمفضلة"
    fav_cb = f"fav_rem:{q_id}" if is_fav else f"fav_add:{q_id}"

    nav_row_1 = [
        InlineKeyboardButton(text=fav_text, callback_data=fav_cb),
        InlineKeyboardButton(text="⚠️ خطأ في الشرح", callback_data=f"report_expl_start:{q_id}")
    ]
    
    nav_row_2 = []
    nav_row_2.append(InlineKeyboardButton(text="↩️ العودة", callback_data=f"back_to_corr:{q_id}"))
    current_state = await state.get_state()
    if current_state == QuizStates.answering.state:
        nav_row_2.append(InlineKeyboardButton(text="⏭️ السؤال التالي", callback_data="quiz_next"))
